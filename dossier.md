...
3. **AI claims.** List every AI/ML claim on their landing page or pricing page. For each, find ONE independent source. Mark each VERIFIED, DISPUTED, or UNVERIFIED. If the company has no AI claims, write "none" and move on — do not invent rows.
...
- **Homepage unreachable.** If the homepage returns 403/404 or is fully JS-rendered, retry once via `https://web.archive.org/web/2026/<url>`. If that also fails, mark every step-3 row `UNVERIFIED` with reason `homepage unreachable`, and add `homepage unreachable — claims sourced indirectly` to the "cannot see" section. Do not skip step 3.
